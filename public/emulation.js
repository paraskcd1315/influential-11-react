...
const widgetConfiguration = {
	...
	persistTaskbar: false,
	persistStartscreen: false,
	favAppsTitle: 'Welcome, ParasKCD',
	favAppsBG: false,
	persistFavApps: false,
	borderRadius: 0.0,
	blurRadius: 20.0,
	textColor: '#000000b3',
	backgroundColor: '#ffffff50',
	rippleColor: '#0000004d',
	textColorDark: '#ffffffb3',
	backgroundColorDark: '#1c1c1c96',
	moduleBackgroundColorDark: '#2a2b2c90',
	rippleColorDark: '#ffffff4d',
	allAppsIconSize: 1.0,
	favIconSize: 1.0,
	dockIconSize: 1.0
};
...
const media = {
	nowPlaying: {
		...
		artwork:
			'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACgAAAAoCAYAAACM/rhtAAAABGdBTUEAALGPC/xhBQAAACBjSFJNAAB6JgAAgIQA' +
	...
	nowPlayingApplication: {
		...
		// Base64 is used here just for the sake of being able to include an image directly inside this script
		icon:
			'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEsAAABLCAYAAAA4TnrqAAAEGWlDQ1BrQ0dDb2xvclNwYWNlR2VuZXJpY1JHQgAAOI' +
	...
	london: {
		now: {
			...
			condition: {
				description: 'Fair',
				narrative:
					'Mostly sunny currently. The high will be 14°. Clear tonight with a low of 3°. ',
				code: 34
			},
			ultraviolet: {
				index: 1,
	...
	san_francisco: {
		now: {
			...
			condition: {
				description: 'Cloudy',
				narrative:
					'Partly cloudy today. It’s currently 62°; the high will be 64°. ',
				code: 26
			},
			ultraviolet: {
				index: 2,
	...
	london: {
		now: {
			...
			condition: {
				description: 'Fair',
				narrative:
					'Mostly sunny currently. The high will be 57°. Clear tonight with a low of 38°. ',
				code: 34
			},
			ultraviolet: {
				index: 1,
...
if (window.api !== undefined) {
	console.error(
		'emulation.js :: Detected that Xen Widget API is available, stopping emulation.'
	);
} else {
	...
	var api = {
		...
		media: {
			...
			seekToPosition: function (value) {
				...
				if (value >= api.media.nowPlaying.elapsed)
					value = api.media.nowPlaying.elapsed;
		...
		calendar: {
			...
			fetch: function (start, end, calendarsParam) {
				return new Promise((resolve) => {
					const ids = calendarsParam
						? calendarsParam.map((calendar) => {
								return calendar.id;
						  })
						: [];
					...
					const filteredEvents = events.filter((event) => {
						...
						if (ids.length > 0 && !ids.includes(event.calendar.id))
							return false;
			...
			create: function (params) {
				return new Promise((resolve, reject) => {
					...
					let end =
						params.start && params.end ? params.end : start + 60 * 60 * 1000;
					if (params.allDay) {
	...
	const payload =
		weatherConfig.units === 'imperial'
			? imperial_weather[weatherConfig.city]
			: metric_weather[weatherConfig.city];
	...
	for (let i = 0; i < payload.hourly.length; i++) {
		...
		_date.setHours(
			_date.getHours() + timezoneOffsetGMT.hour,
			_date.getMinutes() + timezoneOffsetGMT.minute
		);
	...
	for (let i = 0; i < payload.daily.length; i++) {
		...
		payload.daily[i].moon.moonrise = datestringToInstance(
			payload.daily[i].moon.moonrise
		);
		payload.daily[i].moon.moonset = datestringToInstance(
			payload.daily[i].moon.moonset
		);
		payload.daily[i].sun.sunrise = datestringToInstance(
			payload.daily[i].sun.sunrise
		);
		payload.daily[i].sun.sunset = datestringToInstance(
			payload.daily[i].sun.sunset
		);
	}

	// `nightly` properties
	for (let i = 0; i < payload.nightly.length; i++) {
		payload.nightly[i].moon.moonrise = datestringToInstance(
			payload.nightly[i].moon.moonrise
		);
		payload.nightly[i].moon.moonset = datestringToInstance(
			payload.nightly[i].moon.moonset
		);
	}
